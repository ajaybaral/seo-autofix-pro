...
class Title_Scanner {
    ...
    public function detect_seo_plugin(): string {
        if ( null !== $this->seo_plugin ) { return $this->seo_plugin; }
    ...
    public function classify( string $title, array &$seen_map ): string {
        $trimmed = trim( $title );
        if ( '' === $trimmed ) { return self::ISSUE_MISSING; }

        $len = mb_strlen( $trimmed );
        if ( $len < self::MIN_LEN ) { return self::ISSUE_SHORT; }
        if ( $len > self::MAX_LEN ) { return self::ISSUE_LONG; }

        // Duplicate: normalise (trim + lowercase + collapse spaces)
        $norm = preg_replace( '/\s+/', ' ', strtolower( $trimmed ) );
        if ( isset( $seen_map[ $norm ] ) ) { return self::ISSUE_DUPLICATE; }
        $seen_map[ $norm ] = true;
    ...
    public function scan_batch( int $batch_size, int $offset, string $post_type = 'any', string $issue_filter = 'all' ): array {
        ...
        foreach ( $query->posts as $post ) {
            ...
            if ( 'all' !== $issue_filter && $issue !== $issue_filter ) { continue; }
